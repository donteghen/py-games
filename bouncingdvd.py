"""Bouncing DVD Logo, by Donald Teghen donaldteghen@gmailcom
A bouncing DVD logo animation. You have to be "of a certain age" to
appreciate this. Press Ctrl-C to stop.
NOTE: Do not resize the terminal window while this program is running.
This code is available at https://learningwithdonny.com/mini-games/bouncingdvd
...
def main():
    ...
    while True:  # Main program loop.
        for logo in logos:  # Handle each logo in the logos list.
            ...
            if cornerBounces == 10:
                sys.exit()
        #displayCount(cornerBounces)

        for logo in logos:
            # Draw the logos at their new location:
            bext.goto(logo[X], logo[Y])
            bext.fg(logo[COLOR])
            print('DVD',logo[COUNT], end='')
...
def displayCount(cornerBounces=0):
    # Display number of corner bounces:
    bext.goto(WIDTH - WIDTH//2, HEIGHT-HEIGHT//2)
    bext.fg('white')
...
if __name__ == '__main__':
    try:
        ...
        print()
        print('Bouncing DVD Logo, by Donald Teghen')
